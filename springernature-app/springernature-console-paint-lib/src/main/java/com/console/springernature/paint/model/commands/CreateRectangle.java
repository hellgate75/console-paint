...
/**
 * Command that realize the creation of a rectangle on the Canvas the user required coordinates
...
public class CreateRectangle implements ICommand {
	...
	public String[] getSyntax() {
		return new String[] {"L x1 y1 x2 y2", "Create a new rectangle, whose upper left corner is (x1,y1) and lower right corner is (x2,y2).", "Currently only horizontal or vertical lines are supported."};
	}
	...
	public List<Point2D> execute(ICanvas canvas) {
		if (lastMessage==null) {
			...
			c.setCanvasMatrix(GeometryHelper.getInstance().drawRectangle(matrix, x1, y1, x2, y2));
		}
